...
and prolog_of_cmd cmd =
  match cmd with
    ...
    | Dec dec -> prolog_of_dec dec
    | Return expr -> "ret("^prolog_of_sexpr expr^")"

and prolog_of_dec dec =
  match dec with
      ConstDec (id, t, e) -> "const(\""^id^"\","^(prolog_of_type t)^","^(prolog_of_sexpr e)^")"                       
    | FunDec (id, t, args, e) ->"fun(\""^id^"\","^(prolog_of_type t)^",["^(prolog_of_args args)^"],"^(prolog_of_sexpr e)^")"
    | FunRecDec (id, t, args, e) -> "funrec(\""^id^"\","^(prolog_of_type t)^",["^(prolog_of_args args)^"],"^(prolog_of_sexpr e)^")"
    | VarDec (id, t) -> "var(\""^id^"\",ref("^(prolog_of_type t)^"))"
    | ProcDec (id, args, b) -> "proc(\""^id^"\",["^(prolog_of_argsp args)^"],"^prolog_of_bloc b^")"
    | ProcRecDec (id, args, b) -> "procrec(\""^id^"\",["^(prolog_of_argsp args)^"],"^prolog_of_bloc b^")"
    | FunPDec (id, t, args, b) ->"funp(\""^id^"\","^(prolog_of_type t)^",["^(prolog_of_args args)^"],"^(prolog_of_bloc b)^")"
    | FunRecPDec (id, t, args, b) -> "funrecp(\""^id^"\","^(prolog_of_type t)^",["^(prolog_of_args args)^"],"^(prolog_of_bloc b)^")"
