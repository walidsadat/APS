...
and prolog_of_cmd cmd =
  match cmd with
    ...
    | Return expr -> "return("^prolog_of_sexpr expr^")"
...
and prolog_of_def def =
  match def with
    ...
    | ProcDef (id, args, b) -> "proc(\""^id^"\",["^(prolog_of_argsp args)^"],"^prolog_of_block b^")"
    | RecProcDef (id, args, b) -> "procrec(\""^id^"\",["^(prolog_of_argsp args)^"],"^prolog_of_block b^")"
    | FunProcDef (id, t, args, b) ->"funproc(\""^id^"\","^(prolog_of_type t)^",["^(prolog_of_args args)^"],"^(prolog_of_block b)^")"
    | RecFunProcDef (id, t, args, b) -> "funprocrec(\""^id^"\","^(prolog_of_type t)^",["^(prolog_of_args args)^"],"^(prolog_of_block b)^")"
...
and prolog_of_lvalue lv =
  match lv with
      LvalueId x -> "ident(\""^x^"\")"
    | Lvalue (l, e) -> "nth("^prolog_of_lvalue l^", "^prolog_of_sexpr e^")"
